...
class AEPSCommission(RowInfo):
    distributor = models.ForeignKey(to=ZrUser, related_name='dist_aeps_comm_mappings', null=True, blank=True)
    merchant = models.ForeignKey(to=ZrUser, related_name='merc_aeps_comm_mappings', null=True, blank=True)
    aeps_commission_structure_set = models.IntegerField(null=False, blank=False)
    is_active = models.BooleanField(default=False)
